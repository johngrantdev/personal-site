...
// Revalidate the contents that is associated with this comment
export const revalidateArtifact: AfterChangeHook = async ({
  doc: commentDoc,
  req: { payload },
}) => {
  if (commentDoc._status === 'published' && commentDoc.doc) {
    ...
    if (typeof commentDoc.doc === 'string') {
      ...
      if (parentDoc) {
        revalidate({ payload, collection: 'artifacts', slug: parentDoc.slug })
      } else {
