...
const convertScreensToNumbers = (screens: Record<string, string>) => {
  const result: Record<string, number> = {}
  Object.keys(screens).forEach(key => {
    result[key] = parseInt(screens[key] || '0', 10)
  })
  return result
}
  ...
  useEffect(() => {
    ...
    const updateScreenSize = () => {
      ...
      Object.keys(screen.breakpoints).forEach(key => {
        const bp = (screen.breakpoints as Record<string, number>)[key];
        if (bp !== undefined && width >= bp) {
